    ...
    def get_profile():
        ...
        if not hasattr(user, '_profile_cache'):
            ...
            if not getattr(settings, 'AUTH_PROFILE_MODULE', False):
                raise Exception('You need to set AUTH_PROFILE_MODULE in your project settings')
            try:
                app_label, model_name = settings.AUTH_PROFILE_MODULE.split('.')
            except ValueError:
                raise Exception('app_label and model_name should be separated by a dot in the AUTH_PROFILE_MODULE setting')

            try:
                model = models.get_model(app_label, model_name)
                if model is None:
                    raise Exception('Unable to load the profile model, check AUTH_PROFILE_MODULE in your project settings')
